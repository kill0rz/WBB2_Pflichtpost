...
<?php
	...
	$result = $db->unbuffered_query("SELECT userid,username FROM bb".$n."_users");
	while ($row = $db->fetch_array($result)){
		$optionstring .= "<option value='{$row['userid']}'>{$row['userid']} - {$row['username']}</option>";
		$usercounter++;
	}
...
if(isset($_POST['postidtoinsert']) and trim($_POST['postidtoinsert']) != ''){
	...
	if($postidexists){
		...
		foreach($userstoinsert as $userid){
			...
			while ($row = $db->fetch_array($result)){
				if($userid == $row['userid']) $dorun = true;
			}
			if($dorun){
	...
	}else echo "Den Post mit der ID ".$postidtoinsert." gibt es nicht!";
